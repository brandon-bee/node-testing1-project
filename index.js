...
class Seasons {
   ...
   */
  next() {
    const season = this.seasons[this.nextSeason]
    if (this.nextSeason === 3) {
      this.nextSeason = 0
    } else {
      ++this.nextSeason
    }
...
class Car {
  ...
  drive(distance) {
    if ((distance/this.mpg) > this.tank) {
      const driven = this.tank * this.mpg
   ...
   */
  refuel(gallons) {
    const tankRoom = this.tankSize - this.tank
    if (gallons < tankRoom) {
      this.tank += gallons
    } else {
